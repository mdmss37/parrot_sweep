...
class Game {
  ...
  tileRender(num, tileSize = "large") {
    ...
    rows.forEach((row, i) => {
      for (let j = 1; j <= num; j++) {
        const td = `<td class="${tdClass}" data-row=${i +
          1} data-col=${j}></td>`;
  ...
  initGame({ fieldLen, parrotNum, difficulty }) {
    ...
    field.tds.forEach(td =>
      td.addEventListener(
        "click",
        leftClickHandler.bind(null, field, this.status)
      )
    );
  }
}
...
const leftClickHandler = (field, status, e) => {
  console.log(status);
  if (status.win === true || status.lose === true) {
    return;
  }
  const cell = e.target;
  ...
  if (cellValue === 0) {
    ...
    if (countParrotAround(field.field, x, y) === 0) {
      aroundPos.forEach((pos, i) => {
        ...
        const aroundCell = board.querySelector(
          `tr:nth-child(${x + pos[0]})>td:nth-child(${y + pos[1]})`
        );
        // when aroundCell is not null and not opened, open that cell too
        if (aroundCell !== null && aroundCell.classList.contains("unopened")) {
          openCell(field, aroundCell, +x + pos[0], +y + pos[1]);
        }
      });
    }
  } else if (cellValue <= 8) {
